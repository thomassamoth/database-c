...
void ajout_apreciation (MYSQL *con, struct Utilisateur user)
{
    int i=0;
    char appreciation[1000];
    char request [500];
    printf ("\nA quel eleve voulez-vous donner une apreciation ?");
    fflush (stdin);
    scanf ("%d", &i);

    printf("\n Donner l'apreciation : \n");
    fflush (stdin);
    scanf("%s", &appreciation[1000]);

    int id = get_id(con, user); 	// Récupération de l'id
    sprintf(request, "INSERT INTO Bulletin VALUES(%d, %i);", id, i);
    if (mysql_query(con, request))
    {
        fprintf(stderr, "%s\n", mysql_error(con));
        //return 1;
...
{
    char request [500];
    sprintf(request,"SELECT* bull_appreciation FROM Bulletin");
    if (mysql_query(con, request))
    {
        fprintf(stderr, "%s\n", mysql_error(con));
    }
